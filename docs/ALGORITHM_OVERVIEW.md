...
### 1.3 노출 조건 (OR: 기본 조건 + Safety Trigger)

- **기본 노출:** 경과 시간 ≥ 5초 **그리고** 지문의 35% 이상 글자 읽음 **그리고** 읽은 문장 수 ≥ 2이면 `status: "ready"`, CPM 노출.
- **Safety Trigger (천천히 읽는 사용자 오해 방지):**  
  경과 시간 ≥ 30초 **그리고** 읽은 문장 수 ≥ 2이면, 위 기본 조건을 만족하지 않아도 즉시 `status: "ready"`로 전환하고 CPM을 노출한다.  
  이때는 데이터가 부족할 수 있으므로 초기값(400) 쪽 스무딩 비중을 높여(`SAFETY_TRIGGER_SMOOTH_WEIGHT_NEW = 0.08`) 수치가 과하게 낮게 튀지 않게 보정한다.
- **최소 계산 시간:** 1초 미만이면 0 반환 (0으로 나누기 방지).
- **초반 스무딩:** 읽은 문장 수가 적을 때는 새 측정값 가중치를 낮게 유지하여 급상승을 억제.

### 1.4 시각적 피드백 (측정 중 + 읽은 문장 1개 이하)

- 경과 30초가 지났는데도 읽은 문장이 1개 이하면, 작은 안내 문구 **"천천히 시작해도 괜찮아요."** 를 사이드바에 표시.
- 경과 45초가 지났는데도 읽은 문장이 1개 이하면, **"읽기에 집중해 볼까요?"** 로 안내 문구를 바꿔 표시.
