...
export default function VocabularyTooltip({
  ...
  const popup =
    ...
    createPortal(
      <div
        ref={popupRef}
        className="fixed z-[60] min-w-[120px] overflow-y-auto whitespace-normal rounded-2xl border border-orange-200 bg-white p-5 pb-6 shadow-lg"
        style={{
          left: layout?.left ?? Math.max(VIEWPORT_MARGIN, position.centerX - 120),
          top: layout?.top ?? Math.max(VIEWPORT_MARGIN, position.top - POPUP_GAP - 120),
          maxWidth:
            layout?.maxWidth ??
